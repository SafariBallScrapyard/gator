...
func scrapeFeeds(s *state) {
	...
	if err != nil {
		log.Printf("Could not retrieve feed: %v", err)
		return
	}
...
func scrapeFeed(db *database.Queries, feed database.Feed) {
	...
	if err != nil {
		log.Printf("Could not mark %s feed as fetched: %v", feed.Name, err)
		return
	}
	...
	log.Printf("The %s feed was collected, %v posts found", feed.Name, len(data.Channel.Item))
}
